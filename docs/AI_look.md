...
### Build & Synchronization
        - **Vector 098.2 (Context Binding)**: XML parsing MUST bind to `ksefId`. Use strict `update` on the main Invoice record.
...
| VECTOR 098.1 | ABSOLUTRE DUPLICATE SHIELD | findUnique pre-check in API before any Invoice.create. |
| VECTOR 098.2 | XML CONTEXT BINDING | Strict `update` on `ksefId` during XML parsing. NO create in Phase 2. |
...
| Vector 097 | Retention Vault | FIXED | Retention Vault Audit (Invoice Relation & Popover). | Każda kaucja (Retention) musi posiadać `invoiceId`. W widoku `RetentionVault` obowiązkowo używamy `Popover` do wyświetlania detali faktury ("Tytułem" z P_7). Suma kaucji w Skarbcu musi być obliczana dynamicznie na podstawie rekordów. |
| Vector 098 | KSeF / Logic | FIXED | Duplicate Shield (KSeF Unique Logic & Accrual VAT Shield). | Blokada duplikatów oparta na `ksefId`. Używamy `upsert` w synchronizacji KSeF. VAT Shield (Tax Guard) liczymy metodą memoriałową (Accrual Basis) ze wszystkich faktur (PAID + UNPAID). Strategia Purge: Usuwamy nadmiarowe faktury wraz z ich transakcjami, zostawiając najstarszy wpis. |
| Vector 098.1 | KSeF / Logic | FIXED | Absolute Duplicate Shield Standards. | Wprowadzono rygorystyczny "Pre-check" w potoku synchronizacji KSeF (`findUnique`). Wizjoner otrzymuje precyzyjną informację zwrotną w UI o liczbie zapisanych (`saved`) i pominiętych (`skipped`) rekordów. Blokada duplikatów oparta na `ksefId` działa bez wyjątków – jeśli faktura jest już w bazie, sync dla tego rekordu zostaje natychmiast przerwany. |
| Vector 099 | AI / Engine | FIXED | Smart Enrichment Engine (Automatic Contractor Data Validation). | Każdy przetworzony XML z KSeF musi zostać sprawdzony pod kątem aktualności danych kontrahenta (Adres, Nr Konta Bankowego). Wyciągamy `<NrRB>` z sekcji `<Platnosc>`. Przy wykryciu zmian, system **nie nadpisuje** danych automatycznie, lecz tworzy powiadomienie typu `ENRICHMENT_PROPOSAL`. Powiadomienie musi zawierać obiekt `metadata` z różnicami (Diff: oldValue vs newValue). Wizjoner zatwierdza zmiany jednym kliknięciem (Human-in-the-loop). Pominięcie enrichmentu dla zidentyfikowanych duplikatów (Vector 098). |
